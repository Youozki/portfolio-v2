...
放大到 1.6 倍以上之后判据再收窄一层：真正看得见的只是容器里很窄的一条
（`view.getBoundingClientRect()` 与 `visualViewport.offsetLeft/width` 求交，再除掉画布缩放
换成画布坐标），左右各留那一条宽度的 35%。实测 scale 3 时 justpaper 两条带子藏 4 张。

### 6.10 放大之后还要按可视区停绘【单个块】（2026-09-10）

裁段只能按【段】裁，一段有一屏多高；放大 4 倍时同一段要 16 倍贴图，段级已经压不住。
`makeZoomCull()` 再细一层：`visualViewport.scale ≥ 1.6` 时，把落在可视区外的画布块
（**含横向**——放大后只看得见画布很窄的一条宽度）加 `.is-zoff` → `visibility: hidden`。

- 只在倍数够大时才介入，正常浏览一个 class 都不加，动效参数一个都不碰；
- 用 `visibility` 不用 `display`：盒子、滚动尺寸、入场过渡全保留，缩回去立刻恢复（实测归 0）；
- 坐标仍然只用 `rect` + `visualViewport` 的 `offsetTop/offsetLeft/width/height`，
  **绝不掺 `scrollX/scrollY`**（放大后是双份偏移，会把正在看的东西裁掉）；
- 量不到位置的（所在段已被裁段收起）直接跳过，不去和段级判据打架。

实测手机 390/dpr3、scale 3、滚到中段：justpaper 藏 15/58、oreate 10/53、terabox 10/30、
companion 27/93，四页"看得见却被藏"全为 0，缩回 1 倍全部恢复。
