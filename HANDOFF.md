...
### 6.8 按行入场必须自带"验不过就还原"（2026-09-10）

`js/case-doc.js` 把正文按【量的时候】的断行位置烤死成一行一个 `.doc-line`。字形推进量在
不同引擎、不同栅格化尺度下有零点几像素的差别，放大之后某一行就多出一个字、被挤到下一行，
表现为"一个字单独占一行、整段排版乱掉"。**本机 Chrome 不复现**（1 倍下正好装得下），
所以这类问题只能靠用户在真机放大读正文时发现。

现在的机制是**两道**：

1. **CSS 里给 `.doc-line` 写 `white-space: nowrap`**——烤死的行结构上就折不了，宽出来的
   零点几像素只是看不见的溢出，排版不会散。这是真正解决问题的那一刀。
2. 兜底自检：`linesOk()` 看**溢出**（`scrollWidth > clientWidth + 0.75em`，容差留一个空格的量），
   整行宽出去一大截（例如字体换过之后）才判失败，把整块还原成原文、退回整块渐显；
   在 `load`、字体就绪、+1.2s、以及每次缩放变化时各复查一次。段被收起来时量不到，跳过等下一次。

**曾经踩过的坑：判据不能用 `getClientRects().length === 1`。** `.doc-line` 是 `display: block`，
块盒不管折成几行都只有一个矩形，那版判据恒真，等于完全没在工作——用户连报两次"换行还是没改"
就是这么来的。**本机 Chrome 不复现折行**（1 倍下正好装得下），所以这类判据必须结构上成立，
不能依赖"检测到了再补救"。排版正确性优先于那一档按行入场。

### 6.9 横向长图带在放大时要按可视区停绘（2026-09-10）

用户实测"在 justpaper 以那两条滚动图为中心放大，马上就崩"。这两条带子在画布坐标里
3713 / 2737px 宽，整条都是合成层的绘制内容，放大后按缩放平方涨，是全页最贵的一块。

`initHScroll()` 里现在把卷出可视范围的那几张用 **`visibility: hidden`** 藏掉——盒子还在，
`scrollWidth` 与滚动条滑块宽度都不变（这就是不能用 `display: none` 的原因），
但浏览器不再绘制它。判据只用滚动容器自己的可视区
（`[scrollLeft, scrollLeft + clientWidth]` 之外一定被容器裁掉、看不见），左右各留 1/4 宽度余量，
所以拖动时不会看到空档。实测 visual 带 4 张里藏 2 张，"可见却被藏"恒为 0。
**判据不要掺 `visualViewport.offsetLeft/width`**（见 6.10，试过、已撤）。

### 6.10 "按视觉视口裁单个块"这条路封死了（2026-09-10）

段级裁段只能按段裁，一段有一屏多高，放大 4 倍要 16 倍贴图，段级压不住。试过再细一层：
`visualViewport.scale ≥ 1.6` 时把落在可视区外的画布块（含横向）`visibility: hidden`。
本机模拟 scale 3 判据完全正确——四页藏 15/58、10/53、10/30、27/93，"看得见却被藏"全为 0，
缩回 1 倍全部恢复。**但真机上用户立刻回报"放大之后图片和文字闪烁、消失"，已整段撤回。**

原因：iOS 捏合平移时 `visualViewport.offsetTop/offsetLeft` 并不稳定（Safari 会连布局视口
一起挪、offset 归零），按它算出来的可视区和眼睛看到的对不上，正在看的块被判成离屏藏掉，
手指一动又判回来，于是闪。

**结论性教训：任何"按视觉视口位置"决定绘不绘制的判据，都必须带"与布局视口有交集就绝不裁"
这道保险**——段级裁段（6.4）稳就是因为有这一条。而到了单个块这一层，加上保险就等于什么都
裁不掉，所以这条路没有意义，不要再试第三次（第一次是 `scrollY` 双份偏移，第二次是这次）。
放大这一档的内存要继续省，只剩「内页做手机版式」一条路。

### 6.11 内页手机版式（四个画布页全量，2026-09-11）

`js/case-mobile.js` + `css/case-mobile.css`，挂在 terabox / oreate / justpaper / companion 四页。
767px 以下接管：在 `<html>` 上挂 `is-doc-mobile`（`case-doc.js` 见到就整体 return），
把画布块读成一张表按设计稿 y 排序、重排成一列，然后**把 `.case-doc-wrap` 整棵删掉**
（同一份内容不在内存里存两遍）。回退基线：tag `pre-mobile-layout`。

分派规则：

- 正文（画布 20px/170%）→ 17px/1.7；30px 章节名 → 2rem/300 的手机章节头（发丝线 + 序号）；
  ≤16px → 居中图注。章节间距按桌面比例还原（章节头上方 4.5–6.5rem、标题到正文 2.25–3.25rem）。
  **手机上不再按行切分入场**，6.8 那类烤死排版的问题在这条路上不存在。
- 图分三类：
  1. **纯 `<img>` 且画布宽 ≥ 600 的界面截图** → 逐张铺满一列；宽高比 1.5–3.5 的（一图多屏）
     进 `.case-m-rail` 横向滑动；
  2. **画布自带的横向滚动图 `[data-hscroll]` 与图墙 `.marquee`** → 同样进 rail，
     带子里每项按高度铺开；画布那条蓝条（`[data-hscroll-bar]`）丢掉、由手机版重建；
  3. **其余一切**（渐变卡片、带层级的组合件、拼贴小图）→ 保留设计稿坐标整组等比缩。
- 每条 rail 下面都有一条**蓝色滚动条**（`#2084e9`/`#dddddd`/5.5px/圆角 2.75px，与桌面同色同厚），
  滑不动时自动隐藏。用户明确要求："不然用户不知道能滚动"。

**图片绝不许变形**——三条都是踩出来的：

- 只有真正的 `<img>` 才会被拉到一列宽；非 img 的块一律走等比缩，几何关系分毫不动。
- 每张图都补 `width`/`height` 属性（画布内联尺寸就是素材真实比例）。
- **离屏卸解码换成 1×1 空图时，已加载资源的自然比例会盖掉 width/height 属性推出的比例**，
  图会变成正方形。所以卸载前：按宽度定尺寸的写内联 `aspect-ratio`、按高度定尺寸的（rail 里的）
  写内联 `width`，装回来时撤掉。**并且 srcset 必须一起摘**（留着 srcset，src 不生效，卸不掉）；
  地址要用 `getAttribute('src') || currentSrc`，只有 srcset 没有 src 的图会被存成空地址、永久变白。
- 带子里"容器 + 内部绝对定位"的项**不能清宽度**：容器塌成 0 宽后，里面绝对定位的图会按画布
  原尺寸（1500+px）挂在外面，把布局视口撑宽到 1501，顶部胶囊 `left:50%` 跟着跑偏。
  做法是外框按高度定尺寸并 `overflow:hidden`，内部保持画布坐标整块 `scale`。

**图片档位与内存**：自己挑档（`sharpen()`），**必须把 srcset/sizes 一起摘掉再写 src**
（留着 srcset 浏览器仍按桌面 sizes 算，已加载的小档不会为了变大换一张）。
单张解码预算 4MB——先按预算筛掉过大的档（justpaper 那张 2578×1450 的原图一张 15MB），
再在剩下的里挑够用的，然后**按选中的档给显示尺寸封顶**（宽/高上限 = 档宽 / 2.4 ≈ 0.8 屏幕密度），
所以密度稳定、也不会被拉糊。铺满一列的、画布坐标的、rail 里的图都做离屏卸载。

实测 390×844 dpr3、四页各滚完整页：0 变形、0 横向溢出、0 控制台报错、入场全部触发，
16 个章节链接落点 80–87px（顶栏 84px），解码峰值 terabox 7.0 / oreate 9.9 / justpaper 7.2 /
companion 6.4 MB（不崩的水位是 13MB，且手机版式下正文不需要捏合，没有缩放²那笔账）。
桌面 1440 与改前逐项相同：lines 46/64/39/32、docH 6118/8813/9680/7533。
