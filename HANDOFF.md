...
### 6.11 内页手机版式（试点：terabox，2026-09-10）

`js/case-mobile.js` + `css/case-mobile.css`，只挂在 `case-terabox.html` 上。767px 以下接管：
在 `<html>` 上挂 `is-doc-mobile`（`case-doc.js` 见到就整体 return），把画布块读成一张表按
设计稿 y 排序，重排成一列，然后**把 `.case-doc-wrap` 整棵删掉**（同一份内容不在内存里存两遍）。

分派规则：

- 正文（画布 20px）→ 15px/1.8 的流式段落；30px 的章节名 → 手机版章节头（发丝线 + 序号）；
  ≤16px → 图注居中小字。**手机上不再按行切分入场**，所以 6.8 那类烤死排版的问题在这条路上不存在。
- 图组（设计稿里纵向挨着的底板 + 图算一组）按组里最大那张的画布宽度分两种：
  - ≥ 600px 的**界面截图** → 逐张铺满一列；其中宽高比 1.5–3.5 的（一张图里横着摆了好几屏）
    放进 `.case-m-rail` 卡片内横向滑动、按 46vh 放大，这样不用捏合也看得清；
  - 都比 600px 窄的**拼贴小图**（相纸、图标卡）→ 保留设计稿相对位置，整组等比缩进卡片。
- 图片档位自己挑（`sharpen()`）：**必须把 `srcset`/`sizes` 一起摘掉再写 `src`**——留着 srcset，
  浏览器仍按画布上那个桌面 `sizes`（如 13vw）算，已经加载的小档不会为了变大再换一张。
  目标密度 0.8×dpr（视网膜屏肉眼分不出，解码内存差好几倍）；素材本身不够大的按 0.8 密度封顶居中放，
  不许拉满一列。
- 铺满一列的图离屏换 1×1 空图（先固定 aspect-ratio 再换，布局不跳），判据用 IntersectionObserver
  相对布局视口，**不掺 visualViewport**（6.10）。

实测 390×844 dpr3：3 个章节头 / 8 段正文 / 2 条图注 / 5 张图卡、0 横向溢出、0 控制台报错，
Intro-Problem-Solution-Outcome 四个章节链接全部落在 84px（顶栏高度）、误差 0，
滚到页面底部时仍留在内存里的解码只有 2.0MB（6 张已卸），docH 6431。
桌面 1440 与改前逐项相同（lines 46、docH 6118），因为 CSS 带 `media="(max-width: 767px)"`、
JS 在宽视口直接 return。
