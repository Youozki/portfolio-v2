...
(() => {
  ...
  // 图片一律同时锁 width/height、CSS aspect-ratio 与 object-fit。
  // 只靠自然尺寸不够：原画布里有些图同时写了固定宽高，进手机流式布局后仍可能
  // 被旧尺寸或离屏空图撑成错误比例。三层约束叠在一起，盒子尺寸变化也只能留白，不能拉伸。
  function lockRatio(img, fw, fh) {
    if (!img) return;
    const attrW = num(img.getAttribute('width'));
    const attrH = num(img.getAttribute('height'));
    const w = img.naturalWidth || (attrW && attrH ? attrW : 0) || fw || num(img.style.width);
    const h = img.naturalHeight || (attrW && attrH ? attrH : 0) || fh || num(img.style.height);
    if (w && h) {
      img.setAttribute('width', Math.round(w));
      img.setAttribute('height', Math.round(h));
      img.style.setProperty('--m-ar', w + ' / ' + h);
      return;
    }
    img.addEventListener('load', () => {
      if (img.naturalWidth) {
        ...
        img.style.setProperty('--m-ar', img.naturalWidth + ' / ' + img.naturalHeight);
      }
    }, { once: true });
  }
  ...
  // 组合件里的图片也统一补齐比例约束；它们未必经过上面的截图／横滑分支。
  col.querySelectorAll('img').forEach((img) => lockRatio(img, num(img.style.width), num(img.style.height)));
    ...
    const io = new IntersectionObserver((es) => es.forEach((e) => {
      ...
      if (e.isIntersecting) {
        if (img.dataset.mSrc) {
          ...
          // --m-ar 是图片的永久比例约束；恢复资源时只撤掉离屏占位用的内联比例。
